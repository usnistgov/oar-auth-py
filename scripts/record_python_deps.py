...
omddir = os.path.join(pkgdir, "metadata")
omdscr = os.path.join(omddir, "scripts")
pkgname = os.environ.get('PACKAGE_NAME', 'oar-auth-py')
...
def multibagdep():
    import multibag, bagit
    try:
        ver = multibag.__version__
...
def make_depdata(compname, pkgver):
    ...
    samldep = pysamldep()
    deps[samldep['name']] = OrderedDict([
        ('version', samldep['version']),
        ('dependencies', samldep.get('dependencies', []))
    ])
    data = OrderedDict([
        ("name", compname),
        ("version", pkgver),
