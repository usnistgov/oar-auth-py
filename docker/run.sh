...
volopt="-v ${codedir}:/dev/oar-auth-py"
...
if [ -z "$dodockbuild" ]; then
    if wordin python $comptypes; then
        docker_images_built $PKGNAME/build-test || dodockbuild=1
    fi
fi
...
if wordin python $comptypes; then
    ...
    if wordin build $cmds; then
        # build = makedist
        echo '+' docker run --rm $volopt "${dargs[@]}" "${envargs[@]}" $PKGNAME/build-test makedist \
                        "${args[@]}"  "${pyargs[@]}"
        docker run $ti --rm $volopt "${dargs[@]}" "${envargs[@]}" $PKGNAME/build-test makedist \
               "${args[@]}"  "${pyargs[@]}"
    fi

    if wordin test $cmds; then
        # test = testall
        echo '+' docker run --rm $volopt "${dargs[@]}" "${envargs[@]}" $PKGNAME/build-test testall \
                        "${args[@]}"  "${pyargs[@]}"
        docker run $ti --rm $volopt "${dargs[@]}" "${envargs[@]}" $PKGNAME/build-test testall \
               "${args[@]}"  "${pyargs[@]}"
    fi
    ...
    if wordin shell $cmds; then
        ...
        echo '+' docker run -ti --rm $volopt "${dargs[@]}" "${envargs[@]}" $PKGNAME/build-test $cmd \
                        "${args[@]}"  "${pyargs[@]}"
        exec docker run -ti --rm $volopt "${dargs[@]}" "${envargs[@]}" $PKGNAME/build-test $cmd \
                        "${args[@]}"  "${pyargs[@]}"
    fi
fi
